...
FEATUREFLAGS = -DVARIABLE_INTVL_SUPPORT=1

CFLAGS	= $(OFLAGS) $(EXTRAFLAGS) $(FEATUREFLAGS) $(INCDIRFLAG) $(LIBDIRFLAG) -DVERBOSE=$(VERBOSE) -DMATHACCEL=$(ACCELNUM)
...
ifdef LEAKAGE
OBJ	:= $(OBJ) $(LEAKAGEOBJ)
FEATUREFLAGS	:= $(FEATUREFLAGS) -DENABLE_LEAKAGE=1
endif

ifdef GPGPU
OBJ	:= $(OBJ) $(GPUOBJ)
ifdef GPGPU_SINGLE
FEATUREFLAGS	:= $(FEATUREFLAGS) -DGPGPU=1 -DGPU_SINGLE_PRECISION=1
else
FEATUREFLAGS	:= $(FEATUREFLAGS) -DGPGPU=1
endif
endif
